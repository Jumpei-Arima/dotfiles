...
local function check()
    ...
    for _, name in ipairs({'nvim-tree', 'telescope', 'diffview', 'lualine', 'packer'}) do
    ...
    assert(vim.fn.maparg('<C-n>', 'n'):find('NvimTreeToggle'))
    ...
    assert(vim.wait(20000, function() return vim.g.dotfiles_preview_url ~= nil end, 100), 'Preview URL timeout')
    -- Keep Neovim's RPC loop available while the preview server requests data.
    local html, http_exit = '', nil
    local job = vim.fn.jobstart({'curl', '--fail', '--silent', '--show-error', '--max-time', '15', vim.g.dotfiles_preview_url}, {
        stdout_buffered = true,
        on_stdout = function(_, data) html = table.concat(data, '\n') end,
        on_exit = function(_, code) http_exit = code end,
    })
    assert(job > 0)
    assert(vim.wait(20000, function() return http_exit ~= nil end, 50), 'HTTP job timeout')
    assert(http_exit == 0, 'Preview HTTP failed: ' .. tostring(http_exit))
    assert(html:lower():find('<!doctype html'), 'Expected HTML response')
    print('Startup, modules, NvimTree, keymaps, Markdown highlighting and preview HTTP: PASS')
